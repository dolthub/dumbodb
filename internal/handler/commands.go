...
func (h *Handler) initCommands() {
	...
	h.commands = map[string]*Command{
		...
		"create":                   {Handler: h.MsgCreate, MutatesState: alwaysMutatesState, Help: "Creates the collection."},
		...
		"dropAllUsersFromDatabase": {Handler: h.MsgDropAllUsersFromDatabase, MutatesState: alwaysMutatesState, Help: "Drops all users from database."},
		"dropUser":                 {Handler: h.MsgDropUser, MutatesState: alwaysMutatesState, Help: "Drops user."},
		"updateUser":               {Handler: h.MsgUpdateUser, MutatesState: alwaysMutatesState, Help: "Updates user."},
		"usersInfo":                {Handler: h.MsgUsersInfo, Help: "Returns information about users."},
		"grantRolesToUser":         {Handler: h.MsgGrantRolesToUser, MutatesState: alwaysMutatesState, Help: "Grants roles to a user."},
		"revokeRolesFromUser":      {Handler: h.MsgRevokeRolesFromUser, MutatesState: alwaysMutatesState, Help: "Revokes roles from a user."},
		"createRole":               {Handler: h.MsgCreateRole, MutatesState: alwaysMutatesState, Help: "Creates a new user-defined role."},
		"updateRole":               {Handler: h.MsgUpdateRole, MutatesState: alwaysMutatesState, Help: "Updates a user-defined role."},
		"dropRole":                 {Handler: h.MsgDropRole, MutatesState: alwaysMutatesState, Help: "Drops a user-defined role."},
		"dropAllRolesFromDatabase": {Handler: h.MsgDropAllRolesFromDatabase, MutatesState: alwaysMutatesState, Help: "Drops all user-defined roles from database."},
		"grantPrivilegesToRole":    {Handler: h.MsgGrantPrivilegesToRole, MutatesState: alwaysMutatesState, Help: "Grants privileges to a user-defined role."},
		"revokePrivilegesFromRole": {Handler: h.MsgRevokePrivilegesFromRole, MutatesState: alwaysMutatesState, Help: "Revokes privileges from a user-defined role."},
		"grantRolesToRole":         {Handler: h.MsgGrantRolesToRole, MutatesState: alwaysMutatesState, Help: "Grants inherited roles to a user-defined role."},
		"revokeRolesFromRole":      {Handler: h.MsgRevokeRolesFromRole, MutatesState: alwaysMutatesState, Help: "Revokes inherited roles from a user-defined role."},
	...
	h.register(&Command{Handler: h.MsgDumboDBBranch, MutatesState: branchMutatesState, Help: "Creates or deletes a DumboDB branch, or with no branch name lists every branch."}, "doltBranch", "dumboBranch")
	...
	h.register(&Command{Handler: h.MsgDumboDBCherryPick, MutatesState: alwaysMutatesState, Help: "Applies the diff introduced by the named commit onto the current branch encoded in the database name."}, "doltCherryPick", "dumboCherryPick")
	...
	h.register(&Command{Handler: h.MsgDumboDBMerge, MutatesState: alwaysMutatesState, Help: "Merges a source branch into the branch encoded in the database name."}, "doltMerge", "dumboMerge")
	h.register(&Command{Handler: h.MsgDumboDBRebase, MutatesState: alwaysMutatesState, Help: "Reapplies commits on the current branch onto the tip of another branch, rewriting history."}, "doltRebase", "dumboRebase")
	h.register(&Command{Handler: h.MsgDumboDBReset, MutatesState: alwaysMutatesState, Help: "Resets the branch HEAD to a target commit, optionally resetting the working tree."}, "doltReset", "dumboReset")
	h.register(&Command{Handler: h.MsgDumboDBRemote, MutatesState: remoteMutatesState, Help: "Adds, lists, or removes a named remote (name + url) for the database. Stored in admin.system.remotes."}, "doltRemote", "dumboRemote")
	h.register(&Command{Handler: h.MsgDumboDBPush, MutatesState: alwaysMutatesState, Help: "Pushes a branch's committed HEAD to a configured remote (reuses Dolt push)."}, "doltPush", "dumboPush")
	h.register(&Command{Handler: h.MsgDumboDBFetch, MutatesState: alwaysMutatesState, Help: "Fetches a branch from a configured remote and updates the local tracking ref (reuses Dolt fetch)."}, "doltFetch", "dumboFetch")
	h.register(&Command{Handler: h.MsgDumboDBPull, MutatesState: alwaysMutatesState, Help: "Fetches from a remote and merges the fetched commit into the current branch (git pull)."}, "doltPull", "dumboPull")
	h.register(&Command{Handler: h.MsgDumboDBClone, MutatesState: alwaysMutatesState, Help: "Creates a new database by cloning a file:// remote. Run against the admin database."}, "doltClone", "dumboClone")
	h.register(&Command{Handler: h.MsgDumboDBResolveConflict, MutatesState: alwaysMutatesState, Help: "Resolves a single conflict (document, view, metadata, or validation) in the current in-progress merge, cherry-pick, or rebase."}, "doltResolveConflict", "dumboResolveConflict")
	h.register(&Command{Handler: h.MsgDumboDBRevert, MutatesState: alwaysMutatesState, Help: "Reverts the changes introduced by the named commit, creating a new inverse commit."}, "doltRevert", "dumboRevert")
	...
	h.register(&Command{Handler: h.MsgDumboDBTag, MutatesState: tagMutatesState, Help: "Creates, lists, or deletes tags. Tags share the dolt tag refspec (refs/tags/<name>)."}, "doltTag", "dumboTag")
	h.register(&Command{Handler: h.MsgDumboDBUndrop, MutatesState: undropMutatesState, Help: "Restores a soft-deleted database, or with no name lists databases available to undrop. Admin-only."}, "doltUndrop", "dumboUndrop")

	// Durable boundaries: routed through Shadow.Commit.
	h.register(&Command{Handler: h.MsgDumboDBCommit, Durable: true, MutatesState: alwaysMutatesState, Help: "Commits the current working set on the branch encoded in the database name."}, "doltCommit", "dumboCommit")
	h.register(&Command{Handler: h.MsgCommitTransaction, Durable: true, MutatesState: alwaysMutatesState, Help: "Commits a MongoDB transaction."}, "commitTransaction")
	h.register(&Command{Handler: h.MsgDumboDBGC, Durable: true, MutatesState: alwaysMutatesState, Help: "Runs garbage collection on the database's chunk store. Optional mode: \"default\" or \"full\"."}, "doltGC", "dumboGC")

	// Commands rejected with code 263 OperationNotSupportedInTransaction.
	h.register(&Command{Handler: h.MsgDrop, BlockedInTxn: true, MutatesState: alwaysMutatesState, Help: "Drops the collection."}, "drop")
	h.register(&Command{Handler: h.MsgDropDatabase, BlockedInTxn: true, MutatesState: alwaysMutatesState, Help: "Drops production database."}, "dropDatabase")
...
func aggregateMutatesState(msg *wire.OpMsg) bool {
	document, err := opMsgDocument(msg)
	if err != nil {
		return false
	}
	pipelineValue, _ := document.Get("pipeline")
	pipeline, ok := pipelineValue.(*types.Array)
	if !ok {
		return false
	}
	for index := 0; index < pipeline.Len(); index++ {
		stageValue, err := pipeline.Get(index)
		if err != nil {
			return false
		}
		stage, ok := stageValue.(*types.Document)
		if ok && (stage.Has("$out") || stage.Has("$merge")) {
			return true
		}
	}
	return false
...
func branchMutatesState(msg *wire.OpMsg) bool {
	return stringFieldDiffers(msg, "action", "list")
}

func remoteMutatesState(msg *wire.OpMsg) bool {
	return stringFieldDiffers(msg, "action", "list")
}

func tagMutatesState(msg *wire.OpMsg) bool {
	return messageHasAnyField(msg, "name", "delete")
}

func undropMutatesState(msg *wire.OpMsg) bool {
	return messageHasAnyField(msg, "name", "purgeMatching")
}

func stringFieldDiffers(msg *wire.OpMsg, field, readValue string) bool {
	document, err := opMsgDocument(msg)
	if err != nil {
		return false
	}
	...
	return ok && actual != readValue
}

func messageHasAnyField(msg *wire.OpMsg, fields ...string) bool {
	document, err := opMsgDocument(msg)
	if err != nil {
		return false
	}
