...
func (b *Backend) remoteDBParams(scheme string) (map[string]interface{}, error) {
	...
	if isGitScheme(scheme) {
		// Git remotes keep a per-remote bare-repo cache under a server-owned
		// directory. Dolt uses git_cache_root verbatim, creating
		// <cacheRoot>/<remote-hash>/repo.git on demand.
...
// git_cache_root for git remotes. Dolt uses it verbatim, creating
// <gitRemoteCacheRoot>/<remote-hash>/repo.git beneath it -- a real top-level
// directory under the data dir, not a hidden .dolt-nested one.
