...
func TestOnSessionEndReleasesAllLocks(t *testing.T) {
	...
	require.NoError(t, mMain.Acquire("ownerX", "col", []hash.Hash{idA}, true))
	require.NoError(t, mFeat.Acquire("ownerX", "col", []hash.Hash{idB}, true))

	require.NoError(t, mMain.Acquire("ownerY", "col", []hash.Hash{idC}, true))
...
func TestOnSessionEndUnknownOwnerIsNoop(t *testing.T) {
	...
	require.NoError(t, m.Acquire("ownerX", "col", []hash.Hash{idA}, true))
	be.OnSessionEnd("ownerZ")
