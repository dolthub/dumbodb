...
func TestDumboDBPushFetch_HTTPRoundTrip(t *testing.T) {
	...
	res, err := src.DumboDBPush(ctx, &backends.PushParams{DBName: "srcdb", Remote: "origin", RefSpec: "main"})
	if err != nil {
	...
	if _, err := src.DumboDBPush(ctx, &backends.PushParams{DBName: "srcdb", Remote: "origin", RefSpec: "main"}); err != nil {
