# docudoltCherryPick Verification

Manual verification guide for `docudoltCherryPick` end-to-end behavior. Work through each
...
db.getSiblingDB("pickdb__main").runCommand({ docudoltBranch: 1, branch: "feature" })
// Expected: { branch: "feature", ok: 1 }

// Advance feature with a commit we will cherry-pick onto main.
db.getSiblingDB("pickdb__feature").items.insertOne({ _id: 2, v: 2 })
const r2 = db.getSiblingDB("pickdb__feature").runCommand({ docudoltCommit: 1, message: "add-two", author: "bob <bob@docudolt>" })
printjson(r2)
...
```js
const rPick1 = db.getSiblingDB("pickdb__main").runCommand({ docudoltCherryPick: 1, commit: hashC2 })
printjson(rPick1)
...
```js
const log = db.getSiblingDB("pickdb__main").runCommand({ docudoltLog: 1, limit: 2 })
...
db.getSiblingDB("pickdb__feature").items.insertOne({ _id: 3, v: 3 })
const r3 = db.getSiblingDB("pickdb__feature").runCommand({ docudoltCommit: 1, message: "add-three", author: "carol <carol@docudolt>" })
const hashC3feat = r3.commitId

// Cherry-pick C3 onto main with a custom message and author.
const rPick2 = db.getSiblingDB("pickdb__main").runCommand({
  docudoltCherryPick: 1,
  commit: hashC3feat,
...
db.getSiblingDB("pickdb__feature").items.updateOne({ _id: 1 }, { $set: { v: 99 } })
const r4 = db.getSiblingDB("pickdb__feature").runCommand({ docudoltCommit: 1, message: "conflict-source" })
const hashC4feat = r4.commitId

// Modify _id:1 on main too (independently).
db.items.updateOne({ _id: 1 }, { $set: { v: 100 } })
db.runCommand({ docudoltCommit: 1, message: "conflict-target" })

// Cherry-pick the conflicting feature commit onto main.
const rConflict = db.getSiblingDB("pickdb__main").runCommand({ docudoltCherryPick: 1, commit: hashC4feat })
printjson(rConflict)
// Expected: { conflicts: [ { collection: "items", count: 1 } ], ok: 0, code: <N>, errmsg: "docudoltCherryPick: unresolved conflicts in 1 collection(s)" }
```
...
- `errmsg` contains `"docudoltCherryPick"`
...
```js
// Inspect conflicts.
const rConflicts = db.getSiblingDB("pickdb__main").runCommand({ docudoltConflicts: 1, collection: "items" })
...
const rResolve = db.getSiblingDB("pickdb__main").runCommand({
...
const rContinue = db.getSiblingDB("pickdb__main").runCommand({ docudoltCherryPick: 1, continue: 1 })
printjson(rContinue)
// Expected: { commitId: "<hash>", message: "conflict-source\n\n(cherry picked from commit <hashC4feat>)", ok: 1 }
```

Key checks:
- After resolve, `docudoltConflicts` returns empty conflicts
...
db.getSiblingDB("pickdb__feature").items.updateOne({ _id: 1 }, { $set: { v: 200 } })
const r5 = db.getSiblingDB("pickdb__feature").runCommand({ docudoltCommit: 1, message: "another-conflict" })
const hashConflict2 = r5.commitId

// Modify _id:1 on main to create conflict.
db.items.updateOne({ _id: 1 }, { $set: { v: 201 } })
db.runCommand({ docudoltCommit: 1, message: "another-conflict-target" })

// Cherry-pick — expect conflict.
const rConflict2 = db.getSiblingDB("pickdb__main").runCommand({ docudoltCherryPick: 1, commit: hashConflict2 })
// Expected: ok: 0, conflicts array non-empty

// Abort: restore pre-cherry-pick state.
const rAbort = db.getSiblingDB("pickdb__main").runCommand({ docudoltCherryPick: 1, abort: 1 })
printjson(rAbort)
...
- `docudoltConflicts` after abort returns an error (no operation in progress)
...
```js
db.getSiblingDB("pickdb__main").runCommand({ docudoltCherryPick: 1 })
// Expected: ok: 0, errmsg mentions "commit" or "required"
```

**Abort when no cherry-pick in progress:**
```js
db.getSiblingDB("pickdb__main").runCommand({ docudoltCherryPick: 1, abort: 1 })
// Expected: ok: 0, errmsg: "no cherry-pick in progress to abort"
```

**Continue when no cherry-pick in progress:**
```js
db.getSiblingDB("pickdb__main").runCommand({ docudoltCherryPick: 1, continue: 1 })
