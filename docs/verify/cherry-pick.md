...
db.getSiblingDB("pickdb__d_main").runCommand({ docudoltBranch: 1, branch: "feature" })
...
const r2 = db.getSiblingDB("pickdb__d_feature").runCommand({ docudoltCommit: 1, message: "add-two", author: "bob <bob@docudolt>" })
printjson(r2)
...
```js
const rPick1 = db.getSiblingDB("pickdb__d_main").runCommand({ docudoltCherryPick: 1, commit: hashC2 })
printjson(rPick1)
...
```js
const log = db.getSiblingDB("pickdb__d_main").runCommand({ docudoltLog: 1, limit: 2 })
...
const r3 = db.getSiblingDB("pickdb__d_feature").runCommand({ docudoltCommit: 1, message: "add-three", author: "carol <carol@docudolt>" })
...
const r4 = db.getSiblingDB("pickdb__d_feature").runCommand({ docudoltCommit: 1, message: "conflict-source" })
...
const rConflict = db.getSiblingDB("pickdb__d_main").runCommand({ docudoltCherryPick: 1, commit: hashC4feat })
printjson(rConflict)
...
```js
// Inspect conflicts.
const rConflicts = db.getSiblingDB("pickdb__d_main").runCommand({ docudoltConflicts: 1, collection: "items" })
...
const rContinue = db.getSiblingDB("pickdb__d_main").runCommand({ docudoltCherryPick: 1, continue: 1 })
printjson(rContinue)
...
const r5 = db.getSiblingDB("pickdb__d_feature").runCommand({ docudoltCommit: 1, message: "another-conflict" })
...
const rConflict2 = db.getSiblingDB("pickdb__d_main").runCommand({ docudoltCherryPick: 1, commit: hashConflict2 })
// Expected: ok: 0, conflicts array non-empty

// Abort: restore pre-cherry-pick state.
const rAbort = db.getSiblingDB("pickdb__d_main").runCommand({ docudoltCherryPick: 1, abort: 1 })
printjson(rAbort)
...
```js
db.getSiblingDB("pickdb__d_main").runCommand({ docudoltCherryPick: 1 })
// Expected: ok: 0, errmsg mentions "commit" or "required"
```

**Abort when no cherry-pick in progress:**
```js
db.getSiblingDB("pickdb__d_main").runCommand({ docudoltCherryPick: 1, abort: 1 })
// Expected: ok: 0, errmsg: "no cherry-pick in progress to abort"
```

**Continue when no cherry-pick in progress:**
```js
db.getSiblingDB("pickdb__d_main").runCommand({ docudoltCherryPick: 1, continue: 1 })
