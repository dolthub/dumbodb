...
db.getSiblingDB("branchdb@main").runCommand({ doltBranch: 1, branch: "feature" })
...
```js
db.getSiblingDB("branchdb@main").runCommand({ doltCurrentBranch: 1 })
// Expected: { branch: "main", ok: 1 }
```
...
```js
db.getSiblingDB("branchdb@feature").runCommand({ doltCurrentBranch: 1 })
// Expected: { branch: "feature", ok: 1 }
```
...
```js
db.getSiblingDB("branchdb@" + hash1).runCommand({ doltCurrentBranch: 1 })
...
```js
db.getSiblingDB("branchdb@main~1").runCommand({ doltCurrentBranch: 1 })
// Expected error (code 96):
//   MongoServerError[OperationFailed]: doltCurrentBranch: no current branch name
//   (connection is at a specific commit, not a named branch)
```
...
| `mydb@main` | `{ branch: "main", ok: 1 }` |
| `mydb@feature` | `{ branch: "feature", ok: 1 }` |
| `mydb@<hash>` | OperationFailed (code 96) |
| `mydb@main~1` | OperationFailed (code 96) |
