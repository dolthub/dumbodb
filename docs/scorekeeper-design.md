...
results from the `dongo-scorecard` GitHub Actions workflow. After each push to
...
**Decision**: Poll `gh api repos/dolthub/dongo/actions/workflows/<id>/runs` to
...
**Decision**: One bd issue per failing test, in the dongo rig, with label
`scorekeeper`.
...
git -C <dongo-repo-path> log --oneline -5 --format="%H %ae %s" origin/main
```
...
```bash
gt nudge dongo/polecats/<name> "Regression: TestFoo started failing. Likely from your commit <sha>: '<msg>'. Please investigate."
```
...
The plugin is at `/home/ubuntu/dongo/plugins/scorekeeper-dog/plugin.md`.
...
initial baseline for the dongo rig's failing tests.
