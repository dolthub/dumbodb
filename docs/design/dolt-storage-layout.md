...
DumboDB uses Dolt's NBS (Noms Block Store) as its storage engine, structured as a
...
(RootValue) and `"DGRV"` (DoltgresRootValue). When dumbodb stores an ADRM hash
...
dumbodb stores an ADRM chunk directly.
...
for a clean-state comparison. As long as dumbodb always keeps HEAD commit RTVL ==
...
func commitCollectionsAM(ctx context.Context, doltDB datas.Database, vs *dolttypes.ValueStore, ds datas.Dataset, am prolly.AddressMap, desc string) (datas.Dataset, prolly.AddressMap, error) {
    ...
    meta, err := datas.NewCommitMeta("dumbodb", "dumbodb@localhost", desc)
    // ...
