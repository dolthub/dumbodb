# DumboDB Version Control Features: Research & Design Proposal
...
NBS chunk store (content-addressed by SHA-256)
  ...
  Table (prolly.Map)
    ...
    value: row bytes (BSON for DumboDB)
```
...
**DumboDB is a presentation layer. Dolt does the work.**

This is a hard requirement, not a preference. Every version control feature in
DumboDB must be backed directly by an existing Dolt abstraction. DumboDB's job is to
translate MongoDB wire protocol commands into calls to Dolt's Go APIs, then
translate the results back into BSON responses. DumboDB must not reimplement
...
DumboDB calls the Go API layer — the same interfaces the Dolt CLI and SQL engine
themselves use. Do not construct SQL query strings in DumboDB VC handlers.
...
When designing a DumboDB VC feature, the question is always: **"Which Dolt Go API
already does this?"** — not "How do we implement it?" If no Dolt abstraction
exists, escalate to the Dolt team rather than building it in DumboDB.

---

## 4. The DumboDB Opportunity — What to Expose

DumboDB sits at a unique intersection: MongoDB wire protocol (document store UX)
...
### Current DumboDB Versioning State

DumboDB already implements these commands (all behind the `doltXxx` namespace):
...
The following Git/Dolt capabilities don't have DumboDB equivalents yet:
...
5. **Remote push/pull**: No cross-server synchronization. Can't clone a DumboDB
...
surface and the **existing Dolt SQL primitive** that backs it. DumboDB translates;
Dolt executes. No VC logic should be reimplemented in DumboDB.
...
the database/table interfaces (backed by `dolt_history_$TABLE` internally). DumboDB
...
not O(all commits). This is Dolt's core design — DumboDB gets it for free.
...
`dolt_blame_$TABLE` internally). DumboDB calls those methods and translates the
...
**What**: Synchronize a DumboDB database with a remote DumboDB or DoltHub instance.
...
over the wire, update remote's refsAM. DumboDB can reuse Dolt's existing push/pull
...
bare branch name (or omitted). Everything else is read-only. DumboDB enforces this
...
`dolt_conflicts_$TABLE` and `DOLT_CONFLICTS_RESOLVE`). DumboDB's work is purely
presentation:
...
and ref format), DumboDB databases become compatible with DoltHub — the largest public
database sharing platform. A DumboDB user could `doltPush` to DoltHub and browse
...
> "I have a dev DumboDB instance and a prod DumboDB instance. I push tested changes
...
DumboDB's version control foundation is solid. The Dolt storage layer already
...
3. **Document history** — turns DumboDB into an audit log database without any
   external tooling.

Together these three make DumboDB genuinely useful as "version-controlled MongoDB"
