...
GitHub Actions workflow in `dolthub/dongo-parity-tesing`. After each completed
...
The parity workflow tests dongo against a reference Dolt implementation. Unlike
...
```json
{
  ...
  "last_run_url": "https://github.com/dolthub/dongo-parity-tesing/actions/runs/...",
...
**Decision**: Uses `GH_TOKEN` from `/home/ubuntu/.gh_gt_token_dongo_vefify`.

The `dolthub/dongo-parity-tesing` repo requires a separate token with access to
...
Parity failures indicate dongo behavior diverges from the reference Dolt
...
If dongo fails to start during the parity run, that's a distinct failure mode
from a test failure. The bead description notes this when detected, aiding
diagnosis.

## Implementation

The plugin is at `/home/ubuntu/dongo/plugins/parity-scorekeeper-dog/plugin.md`.
...
4. Check for dongo startup failure in logs
...
|---------|----------------|------------------------|
| Repo | dolthub/dongo | dolthub/dongo-parity-tesing |
| Workflow | dongo-scorecard.yml | parity.yml |
