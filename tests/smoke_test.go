...
// (DongoFull)
//
// Dongo does not provide multi-document ACID isolation, but the driver-level
...
// the server. Because Dongo does not provide ACID isolation, the inserted
// documents remain visible — but the server must not panic or return an error
// on abortTransaction. (DongoFull)
func TestTransactionSmoke_Abort(t *testing.T) {
	...
	// AbortTransaction must not return an error even though Dongo does not
