...
func TestRevertVerify(t *testing.T) {
	...
	t.Run("Scenario2_CustomMessageAndAuthor", func(t *testing.T) {
		...
		require.NoError(t, err)
		hashC3 := dumboDBCommit(t, env, dbName+"@main", "add-three", "carol <carol@startup.dev>")
	...
	t.Run("Scenario3_ConflictResponse", func(t *testing.T) {
		...
		require.NoError(t, err)
		hashConflictCommit = dumboDBCommit(t, env, dbName+"@conflict-feat", "add-ten", "alice <alice@acme.com>")
		...
		assert.Equal(t, true, statusRaw["dirty"], "workspace must be dirty during revert conflict")
		assert.Nil(t, statusRaw["commitId"], "commitId must be absent during revert conflict")
	...
	t.Run("Scenario4_ResolveAndContinue", func(t *testing.T) {
		...
		assert.Equal(t, fmt.Sprintf("branch 'main' (ours) modified document 10; commit '%s' (theirs) deleted it", hashConflictCommit),
